import { useState, useEffect } from "react";
import { useNavigate, Link } from "react-router-dom";
import { getMyPayments } from "../../api/paymentApi";
import {
    FiClock, FiCheckCircle, FiXCircle, FiAlertCircle,
    FiSearch, FiChevronRight, FiFilter, FiCreditCard
} from "react-icons/fi";

const paymentStatusConfig = {
    pending: { label: "Menunggu", color: "#f59e0b", bg: "#fef3c7", icon: FiClock },
    paid: { label: "Lunas", color: "#10b981", bg: "#d1fae5", icon: FiCheckCircle },
    failed: { label: "Gagal", color: "#ef4444", bg: "#fee2e2", icon: FiXCircle },
    expired: { label: "Kadaluarsa", color: "#6b7280", bg: "#f3f4f6", icon: FiAlertCircle },
    refund: { label: "Refund", color: "#8b5cf6", bg: "#ede9fe", icon: FiAlertCircle },
};

const filterOptions = [
    { value: "", label: "Semua Status" },
    { value: "pending", label: "Menunggu" },
    { value: "paid", label: "Lunas" },
    { value: "failed", label: "Gagal" },
    { value: "expired", label: "Kadaluarsa" },
    { value: "refund", label: "Refund" },
];

function RiwayatPembayaran() {
    const navigate = useNavigate();
    const [payments, setPayments] = useState([]);
    const [loading, setLoading] = useState(true);
    const [filter, setFilter] = useState("");

    useEffect(() => {
        getMyPayments({ status: filter || undefined })
            .then(res => setPayments(res.data || []))
            .catch(() => {})
            .finally(() => setLoading(false));
    }, [filter]);

    const formatDate = (dateStr) => {
        if (!dateStr) return "-";
        return new Date(dateStr).toLocaleDateString("id-ID", {
            weekday: "short", day: "numeric", month: "short", year: "numeric",
            hour: "2-digit", minute: "2-digit"
        });
    };

    return (
        <div style={{ minHeight: "100vh", background: "#f8fafc", fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
            <div style={{ maxWidth: 800, margin: "0 auto", padding: "32px 20px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
                    <div>
                        <h1 style={{ margin: 0, fontSize: 24, fontWeight: 800 }}>Riwayat Pembayaran</h1>
                        <p style={{ margin: "4px 0 0", fontSize: 14, color: "#667085" }}>
                            {loading ? "Memuat..." : `${payments.length} transaksi`}
                        </p>
                    </div>
                    <button onClick={() => navigate("/user/cari-tukang")} style={{ display: "flex", alignItems: "center", gap: 6, padding: "10px 20px", background: "#026b5e", color: "white", border: "none", borderRadius: 12, fontWeight: 600, fontSize: 14, cursor: "pointer" }}>
                        <FiSearch /> Cari Tukang
                    </button>
                </div>

                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16, background: "white", padding: "12px 16px", borderRadius: 14, border: "1px solid #e4e7ec" }}>
                    <FiFilter style={{ color: "#98a2b3", flexShrink: 0 }} />
                    <select value={filter} onChange={e => setFilter(e.target.value)} style={{ border: "none", outline: "none", width: "100%", fontSize: 14, background: "transparent" }}>
                        {filterOptions.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                    </select>
                </div>

                {loading ? (
                    <div style={{ textAlign: "center", padding: 60, color: "#667085" }}>Memuat data...</div>
                ) : payments.length === 0 ? (
                    <div style={{ textAlign: "center", padding: "60px 20px", background: "white", borderRadius: 20, border: "1px solid #e4e7ec" }}>
                        <FiCreditCard style={{ fontSize: 48, color: "#d1d5db", marginBottom: 16 }} />
                        <h3 style={{ margin: "0 0 8px", fontWeight: 700 }}>Belum ada pembayaran</h3>
                        <p style={{ margin: "0 0 20px", fontSize: 14, color: "#667085" }}>Belum ada transaksi pembayaran yang tercatat.</p>
                        <button onClick={() => navigate("/user/cari-tukang")} style={{ padding: "10px 24px", background: "#026b5e", color: "white", border: "none", borderRadius: 12, fontWeight: 600, cursor: "pointer" }}>
                            Cari Tukang Sekarang
                        </button>
                    </div>
                ) : (
                    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                        {payments.map(item => {
                            const config = paymentStatusConfig[item.payment_status] || { label: item.payment_status, color: "#6b7280", bg: "#f3f4f6", icon: FiAlertCircle };
                            const Icon = config.icon;
                            return (
                                <Link key={item.id} to={`/user/payment/invoice/${item.invoice_number}`} style={{ textDecoration: "none", color: "inherit" }}>
                                    <div style={{ background: "white", borderRadius: 16, border: "1px solid #e4e7ec", padding: 16, display: "flex", alignItems: "center", gap: 14 }}>
                                        <div style={{ width: 44, height: 44, borderRadius: "50%", background: "#e6f4f2", color: "#026b5e", display: "grid", placeItems: "center", fontWeight: "bold", fontSize: 14, flexShrink: 0 }}>
                                            <FiCreditCard size={20} />
                                        </div>
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                                <h3 style={{ margin: 0, fontSize: 14, fontWeight: 700 }}>{item.invoice_number}</h3>
                                                <span style={{ fontSize: 11, color: "#98a2b3" }}>#{item.booking_id}</span>
                                            </div>
                                            <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 2, fontSize: 12, color: "#98a2b3" }}>
                                                <span>{item.nama_tukang}</span>
                                                <span>·</span>
                                                <span>{item.nama_kategori}</span>
                                                <span>·</span>
                                                <span>{formatDate(item.created_at)}</span>
                                            </div>
                                            <div style={{ fontSize: 13, fontWeight: 700, color: "#026b5e", marginTop: 2 }}>
                                                Rp {parseFloat(item.amount).toLocaleString()}
                                            </div>
                                        </div>
                                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                            <span style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 12px", borderRadius: 16, fontSize: 11, fontWeight: 700, background: config.bg, color: config.color }}>
                                                <Icon size={12} /> {config.label}
                                            </span>
                                            <FiChevronRight style={{ color: "#d1d5db", flexShrink: 0 }} />
                                        </div>
                                    </div>
                                </Link>
                            );
                        })}
                    </div>
                )}
            </div>
        </div>
    );
}

export default RiwayatPembayaran;
