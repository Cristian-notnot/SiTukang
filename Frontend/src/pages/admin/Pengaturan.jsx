...
function PengaturanPage() {
  ...
  const [form, setForm] = useState({
    ...
    email_admin: "halo@situkang.id",
    deskripsi: "Marketplace jasa tukang profesional dan terpercaya.",
  });
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await getPengaturan();
        if (r?.data) {
          const d = r.data;
          setForm((p) => ({ ...p, nama_platform: d.nama_platform || p.nama_platform, email_admin: d.email_admin || p.email_admin, deskripsi: d.deskripsi || p.deskripsi }));
          setInitial(d);
        }
      } catch { }
    })();
  ...
  const handleSave = async () => {
    setLoading(true);
    try {
      await updatePengaturan({ nama_platform: form.nama_platform, email_admin: form.email_admin, deskripsi: form.deskripsi });
      setInitial({ ...form });
      success("Pengaturan berhasil disimpan");
    } catch (e) {
      error(e?.response?.data?.message || "Gagal menyimpan pengaturan");
    }
    setLoading(false);
  };
  ...
  const render = () => {
    if (active === "Umum") return <><Section title="Pengaturan umum"><div className="settings-grid"><Input label="Nama platform" value={form.nama_platform} onChange={(e) => set("nama_platform", e.target.value)} /><Input label="Email support" value={form.email_admin} onChange={(e) => set("email_admin", e.target.value)} /><Input label="Mata uang" defaultValue="IDR" disabled /><Input label="Zona waktu" defaultValue="Asia/Jakarta (WIB)" disabled /><Textarea label="Deskripsi platform" value={form.deskripsi} onChange={(e) => set("deskripsi", e.target.value)} rows={4} /></div></Section><Section title="Mode operasional">{row("regTukang", "Pendaftaran tukang", "Izinkan tukang baru mendaftar")}{row("regCustomer", "Pendaftaran customer", "Izinkan customer baru mendaftar")}{row("maintenance", "Mode pemeliharaan", "Nonaktifkan akses publik sementara")}{row("emailNotif", "Notifikasi email", "Kirim notifikasi via email")}</Section></>;
    if (active === "Brand") return <><Section title="Identitas brand"><div className="settings-grid"><div><div className="logo-preview">ST</div><button className="btn btn-primary" style={{marginTop:12}} onClick={() => toast("Fitur ganti logo akan segera tersedia")}>Ganti logo</button></div><div className="settings-grid"><Input label="Nama brand" defaultValue="SiTukang" disabled /><Input label="Tagline" defaultValue="Solusi Tukang Terpercaya" disabled /><Input label="Font heading" defaultValue="Plus Jakarta Sans" disabled /><Input label="Font body" defaultValue="Inter" disabled /></div></div></Section><Section title="Palet warna"><div className="color-grid">{[["Primary","#F97316"],["Secondary","#0F172A"],["Accent","#FACC15"],["Background","#FFFFFF"]].map(([l,h])=><div className="color-card" key={l}><div className="color-box" style={{background:h}}></div><div className="color-label">{l}</div><div className="color-hex">{h}</div></div>)}</div></Section></>;
    if (active === "Pembayaran") return <><Section title="Metode pembayaran aktif">{row("bank","Transfer Bank","Pembayaran melalui rekening bank")}{row("wallet","E-Wallet","OVO, DANA, GoPay, ShopeePay")}{row("qris","QRIS","Pembayaran QR nasional")}{row("cc","Kartu Kredit","Visa dan Mastercard")}{row("cod","Tunai COD","Pembayaran langsung ke tukang")}</Section><Section title="Konfigurasi gateway"><div className="settings-grid"><Input label="Payment Gateway" defaultValue="Midtrans" disabled /><Input label="Mode" defaultValue="Production" disabled /><Input label="Merchant ID" defaultValue="M123456" disabled /><Input label="Server Key" type="password" defaultValue="server-key" disabled /><Input label="Komisi platform %" type="number" defaultValue="10" disabled /><Input label="Min. penarikan tukang" defaultValue="Rp50.000" disabled /></div></Section></>;
